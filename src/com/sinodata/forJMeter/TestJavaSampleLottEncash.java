...
public class TestJavaSampleLottEncash extends AbstractJavaSamplerClient{
	...
	public Arguments getDefaultParameters() {
		...
		params.addArgument("EncashRunCode", "");
		params.addArgument("TicketCode", "");
		params.addArgument("PlayId", "");
		params.addArgument("Station", "");
		params.addArgument("SellTermCode", "");
		params.addArgument("ValidTermCode", "");
	...
	public SampleResult runTest(JavaSamplerContext arg0) {
		...
		sr.setSampleLabel("Java请求(兑奖(自助终端))");//察看结果树的标题显示
		...
		while (it.hasNext()){
			...
			if ("CheckCode".equals(key)){
				if("default".equals(value)){
					...
					sb.append(arg0.getParameter("EncashRunCode"));
					sb.append(arg0.getParameter("TicketCode"));
		...
		try {
			...
			sr.setRequestHeaders("测试兑奖(自助终端)");
			
			// 通过下面的操作可以将被测方法的响应输出到Jmeter的察看结果树中的响应数据里。
			String uri = "/api/access/lottEncash?";
